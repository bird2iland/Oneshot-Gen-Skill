...
pub fn get_python_path() -> String {
    ...
    // 尝试 2: 使用项目根目录的 .venv（优先）
    let root_venv_python = root_path.join(".venv").join("bin").join("python");
    if root_venv_python.exists() {
        eprintln!("DEBUG: Using root .venv python: {:?}", root_venv_python);
        return root_venv_python.display().to_string();
    }
...
pub fn get_root_path() -> PathBuf {
    ...
    if let Ok(path) = std::env::var("VIDEO_GEN_ROOT_PATH") {
        ...
        // 默认路径：相对于 GUI 目录的父目录的父目录（即项目根目录）
        // cargo run 在 src-tauri 目录执行，所以 current_dir() 是 src-tauri
        // parent() 是 video-gen-gui，再 parent() 才是项目根目录
        let current_dir = std::env::current_dir()
            .unwrap_or_else(|_| PathBuf::from("."));
        eprintln!("DEBUG: Current dir: {:?}", current_dir);
